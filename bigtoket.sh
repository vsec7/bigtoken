#!/usr/bin/env bash
# BIGTOKEN AUTO REGISTER & VERIFICATION (UPDATE)
...
getLink(){
	url=$(curl -sI $1 -H 'user-agent: Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Mobile Safari/537.36')
	echo $url
}
...
bigtoken(){
	...
	if [[ -z $reg ]];then
		...
		sleep $2
		link=$(readEmail "$e" | grep -oP 'none" href="\K[^"]+')
		toket=$(getLink $link | grep -oP '(?<=verify%3Fcode%253D).*?(?=%2526type%253D)')
		if [[ -z $toket ]];then
			echo "[ Failed ]"
			echo $link >> unverif.txt
...
ref="7C70QAO1F" 		# Code Referal kamu
waitingCheckInbox="2" 	# 60s / 1 menit Waktu tunggu untuk membaca inbox
